export type PmMeasurementKind = 'current_3phase' | 'vibration_3axis' | 'none'

const VIBRATION_RE =
  /vibrat|สั่น|แกน\s*[xyz]|mm\/s|rms|velocity|acceleration|ความเร็วสั่น/i
...
export function inferPmMeasurementKind(input: {
  ...
  if (VIBRATION_RE.test(blob)) return 'vibration_3axis'
  return 'none'
}
...
export function pmMeasurementMeta(kind: PmMeasurementKind): {
  ...
  if (kind === 'vibration_3axis') {
    return {
      unit: 'mm/s',
      labels: ['แกน X', 'แกน Y', 'แกน Z'],
      title: 'Vibration 3 แกน',
      subtitle: 'แนวโน้มค่า vibration แต่ละแกนตามเวลา (หน่วย mm/s RMS)',
    }
  }
  return null
}
