...
test.describe('U4f — portal flow', () => {
  ...
  test('seeded session — /portal shows modules', async ({ page, request }) => {
    ...
    await expect(page.getByRole('button', { name: /Open Spare Parts Store/i })).toHaveCount(0)
    await expect(page.getByRole('button', { name: /Open Repair Request/i })).toHaveCount(0)
  })
})
