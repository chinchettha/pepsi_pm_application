...
if (!empty($_SESSION['mem_id'])){
	$strSQL = "SELECT * FROM view_planwork WHERE idwkctr='".$_SESSION['mem_id']."' AND   syst NOT IN ('CRTD', 'REL')  ";
	//echo $strSQL;
}
?>
...
?>
        ...
        <td><?PHP if(!empty($result["cday"])){ echo date("d.m.Y",$result["cday"] ); } ?></td>
                                                <td align="center">
												<a href="<?php $PHP_SELF ?>index2.php?module=M_planwork_view_form_close&id=<?php echo $result['idiw37'];?>" role="button" class="btn btn-outline-info btn-sm btn-edit" ><i class="fa fa-save"></i> บันทึกปิดงาน</a>
												</td>
        </tr>
<?php } ?>
