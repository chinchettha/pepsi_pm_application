...
if ($_REQUEST['op'] == "save") {
     ...
     $sql = "SELECT * FROM $tbl_policy WHERE iddepartment = '" . $_REQUEST['iddepartment'] . "' ";
     ...
     if ($result_chk->num_rows > 0) {
          ...
          $sqlupdate_main .= " department='" . $_REQUEST['department'] . "' ";
          $sqlupdate_main .= " WHERE iddepartment = '" . $_REQUEST['iddepartment'] . "' ";
          $result = $link->query($sqlupdate_main);
		  //echo $sqlupdate_main;
     } else {
          $record_date = date("Y-m-d H:i:s");
          //ดำเนินการเพิ่มข้อมูลลงในตารางหลัก
          $sqlinsert_main = "INSERT $tbl_policy (iddepartment,department) VALUES ('" . $_REQUEST['iddepartment'] . "','" . $_REQUEST['department'] . "')";
...
//หากมีการกดปุ่มลบ
if ($_REQUEST['op'] == "del") {
	 $sql_del = "DELETE FROM $tbl_policy WHERE iddepartment = '" . $_REQUEST['iddepartment'] . "' ";
     $link->query($sql_del);
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover" id="dataTable" width="100%" cellspacing="0" data-page-length='100'>
                                        <thead class="thead-dark">
                                            <tr>
                                                <th><i class='fa fa-key nav-icon'></i>iddepartment</th>
                                                <th>department</th>
                                                <th>Action</th>
                                            </tr>
                                        </thead>
...
?>
                                            <tr>
                                                <td><?php echo $result['iddepartment'];?></td>
                                                <td><?php echo $result['department'];?></td>
                                                <td align="center">
												<a href="pages/<?php echo $tbl_policy;?>_form.php?op=edit&iddepartment=<?php echo $result['iddepartment'];?>" data-id="<?php echo $result['iddepartment']; ?>" data-name="<?php echo $result['department']; ?>" role="button" class="btn btn-outline-dark btn-sm btn-edit" data-toggle="modal" data-target="#ajaxLargeModal"><i class="fa fa-edit"></i> แก้ไข</a>
												<a href="pages/<?php echo $tbl_policy;?>_form.php?op=del&<?php echo $filed1;?>=<?php echo $result[$filed1];?>" data-id="<?php echo $result['iddepartment']; ?>" data-name="<?php echo $result['department']; ?>" role="button" class="btn btn-outline-dark btn-sm btn-edit" data-toggle="modal" data-target="#ajaxLargeModal"><i class="fa fa-trash"></i> ลบ</a>
