...
if($st == "Add") { //เพิ่มข้อมูล Table tbplaningwork
	...
	$strSQL = " SELECT * FROM  tbcofirm  where idiw37='$idiw37' and wkctr='$wkctr' ";
	...
	if($totalRecords >0){ //เช็คว่ามีข้อมูลการย้ายหรือยัง
        ...
        include_once("ShowClose.php");	
	}else{ //ถ้าไม่พบข้อมูลให้เพิ่มใหม่
		$strAdd = " INSERT INTO tbcofirm(`idiw37`, `confirmation`, `wkctr`, `stdate`, `endate`, `cwkctr`, `timeclose`, `timewk`,unitc) 
					VALUES ($idiw37, '', '$wkctr', '$stdate', '$endate', '$wkctrpw', '$dayNow', '$a', '$b') ";
...
if($st == "Del"){ //ลบข้อมูล Table tbplaningwork
	...
	$SQLdel = " DELETE FROM tbcofirm WHERE `idclose` = '$wkctr' ";
...
if($st == "Del_Work"){ //ลบข้อมูล Table tbwrkclose

	$SQLdelW = " DELETE FROM tbwrkclose WHERE `idwrkclose` = '$wkctr' ";
	$queryDelW = mysqli_query($link, $SQLdelW) or die ("Error Query [".$SQLdelW."]");	
		if($queryDelW>0){
            include_once("ShowWorkClose.php");
			//echo "<script> alert('Del Plan Success'); </script>";
		}else{
			echo "<script> alert('Del Confrim Unsuccessful'); </script>";
		}	
} //ลบข้อมูล Table tbwrkclose
